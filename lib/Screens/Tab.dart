...
import 'package:loveafghan/Screens/Profile/profile.dart';
import 'package:loveafghan/Screens/Splash.dart';
import 'package:loveafghan/Screens/blockUserByAdmin.dart';
import 'package:loveafghan/Screens/notifications.dart';
    ...
    _firebaseMessaging.configure(
      onLaunch: (Map<String, dynamic> message) async {
        ...
        if (Platform.isIOS && message['type'] == 'Call') {
          ...
          callInfo['senderID'] = message['senderID'];
        ...
        for (var doc in data.documents) {
          ...
          if (checkedUser.any(
            ...
            if (distance <= currentUser.maxDistance &&
                ...
                !temp.isBlocked) {
              temp.imageUrl
                  .removeWhere((element) => element.approved != 'true');
              if (temp.imageUrl.isNotEmpty) {
                users.add(temp);
              } else {
                print("All Photos are unapproved!");
              }
            }
          }
        }
                          ...
                          title: TabBar(
                              ...
                              tabs: [
                                Tab(
                                  icon: Icon(
                                    Icons.person,
                                    size: 30 + 7.2, // increased size by 30%
                                  ),
                                ),
                                Tab(
                                  icon: Icon(
                                    Icons.whatshot,
                                    size: 24 + 7.2, // increased size by 30%
                                  ),
                                ),
                                Tab(
                                  icon: Icon(
